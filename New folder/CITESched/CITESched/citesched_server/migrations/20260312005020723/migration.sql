...
-- Enable btree_gist for exclusion constraints on bigint
CREATE EXTENSION IF NOT EXISTS btree_gist;

-- Populate schedule_time based on timeslot day/start/end using a fixed anchor week
CREATE OR REPLACE FUNCTION set_schedule_time_range() RETURNS trigger AS $$
DECLARE
    ts record;
    anchor_date date := DATE '2026-03-16';
    day_offset int;
    start_ts timestamptz;
    end_ts timestamptz;
BEGIN
    IF NEW."timeslotId" IS NULL THEN
    ...
    INTO ts
    FROM "timeslot"
    WHERE "id" = NEW."timeslotId";

    IF ts.day IS NULL THEN
        NEW."schedule_time" = NULL;
        RETURN NEW;
    END IF;

    day_offset := CASE lower(ts.day)
        WHEN 'mon' THEN 0
        WHEN 'tue' THEN 1
        WHEN 'wed' THEN 2
        WHEN 'thu' THEN 3
    ...
    start_ts := (anchor_date + day_offset) + (ts."startTime")::time;
    end_ts := (anchor_date + day_offset) + (ts."endTime")::time;
    NEW."schedule_time" = tstzrange(start_ts, end_ts, '[)');
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER schedule_set_time_range
BEFORE INSERT OR UPDATE OF "timeslotId"
ON "schedule"
FOR EACH ROW EXECUTE FUNCTION set_schedule_time_range();

-- Backfill schedule_time for existing rows
UPDATE "schedule" SET "timeslotId" = "timeslotId";

-- Prevent resource overlaps at the database level
ALTER TABLE "schedule"
    ADD CONSTRAINT schedule_instructor_no_overlap
    EXCLUDE USING gist ("facultyId" WITH =, "schedule_time" WITH &&)
    WHERE ("schedule_time" IS NOT NULL);

ALTER TABLE "schedule"
    ADD CONSTRAINT schedule_room_no_overlap
    EXCLUDE USING gist ("roomId" WITH =, "schedule_time" WITH &&)
    WHERE ("roomId" IS NOT NULL AND "schedule_time" IS NOT NULL);

ALTER TABLE "schedule"
    ADD CONSTRAINT schedule_section_no_overlap
    EXCLUDE USING gist ("sectionId" WITH =, "schedule_time" WITH &&)
    WHERE ("sectionId" IS NOT NULL AND "schedule_time" IS NOT NULL);
...
INSERT INTO "serverpod_migrations" ("module", "version", "timestamp")
    VALUES ('citesched', '20260312005020723', now())
    ON CONFLICT ("module")
    DO UPDATE SET "version" = '20260312005020723', "timestamp" = now();
